const renderList = list => {
  // Zero-width space. This is placed before the first underscore so that Mattermost actually italicizes it 
  const zwsp = String.fromCharCode(8203)
  return (list.length === 0 ? "_(none)_" : list.slice(0, 5).join(", ") + (list.length > 5 ? `${zwsp}_, and ${list.length - 5} more..._` : ""))
}
